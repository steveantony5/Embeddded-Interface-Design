...
myMQTTClient.configureEndpoint("aev8ehlr8ljbc-ats.iot.us-east-1.amazonaws.com", 8883)
myMQTTClient.configureCredentials("/home/pi/aws_certificates/Amazon_Root_CA_1.pem", "/home/pi/aws_certificates/3ca4e08fc4-private.pem.key", "/home/pi/aws_certificates/3ca4e08fc4-certificate.pem.crt")
myMQTTClient.connect()
myMQTTClient.publish("my/topic", '"connected"', 0)
...
class mywindow(QtWidgets.QDialog):
    ...
    def periodic_task(self):
        ...
        if timestamp!=0:
            ...
            self.ui.lcd_humidity.display(humidity)
            self.ui.lcd_humidity.repaint()
            payload = '{ "type": "data", "timestamp": ' + str(timestamp) + ', "temperature": ' + str(temp) + ', "humidity": '+ str(humidity) + ' }'
            print (payload)
            myMQTTClient.publish("my/topic", payload, 0)
            ...
            if temp>temp_limit and humidity > hum_limit:
                    ...
                    payload = '{ "type": "Alert", "timestamp": ' + str(timestamp) + ', "Temperature Alert Level": ' + str(temp) + ', "Temperature Trigger Level": '+ str(temp_limit) + ', "Humidity Alert Level": ' + str(humidity) + ', "Humidity Trigger Level": '+ str(hum_limit) + ' }'
                    print (payload)
                    myMQTTClient.publish("my/topic", payload, 0)
            elif temp > temp_limit:
                    print("temp over limit")
                    self.ui.alarm.setText("Temp exceeded limit")
                    payload = '{ "type": "Alert", "timestamp": ' + str(timestamp) + ', "Temperature Alert Level": ' + str(temp) + ', "Temperature Trigger Level": '+ str(temp_limit) + ' }'
                    print (payload)
                    myMQTTClient.publish("my/topic", payload, 0)
            elif humidity > hum_limit:
                    print("humidity over limit")
                    self.ui.alarm.setText("humidity exceeded limit")
                    payload = '{ "type": "Alert", "timestamp": ' + str(timestamp) + ', "Humidity Alert Level": ' + str(humidity) + ', "Humidity Trigger Level": '+ str(hum_limit) + ' }'
                    print (payload)
                    myMQTTClient.publish("my/topic", payload, 0)
            else:
            ...
            payload = '{ "type": "data", "timestamp": ' + str(timestamp) + ', "temperature": "ERROR", "humidity": "ERROR" }'
            print (payload)
            myMQTTClient.publish("my/topic", payload, 0)
